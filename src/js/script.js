...
function waveAnim(el) {
  ...
  if (st + $('.js-viewport').height() >= el.offset().top && st <= el.offset().top) {
    ...
    el.css({x: ((st + $('.js-viewport').height() - el.offset().top)/$('.js-viewport').height())* el.data('value')});
  }
}
...
function parallax(el, target) {
  ...
  if (!target[0].window) {
    target.on('mouseenter', function(e) {
      let settings = parallaxSettings(e, el, target);
      el.transition({
...
$(function () {
  ...
  $('.js-m-banner__slider').each(function () {
    let nav = $(this).next('.js-m-banner__nav'),
        speed = 500,
        autoplaySpeed = 5000,
        progressContainer = nav.find('.js-m-banner__nav-progress')[0];

    let progress = new ProgressBar.Circle(progressContainer, {
      strokeWidth: 2,
      trailWidth: 2,
      trailColor: '#5E7FA2',
      color: '#10EBF8',
  ...
  $('.js-m-news__slider').each(function () {
    let nav = $(this).closest('.js-m-news').find('.js-m-news__nav')
...
$('.js-catalog-equip-slider').each(function () {
  ...
  // modal touchspin
  $("input[id='modal-body__count']").TouchSpin();

  // catalog slider touchspin
  $("input.js-catalog-equip-count").TouchSpin();

  // catalog slider touchspin
  $("input.js-cart__item-count").TouchSpin();
...
//================ УДАЛИТЬ НА СБОРКЕ =========================//
    ...
    // catalog touchspin
    $("input.js-catalog-count").TouchSpin();
  ...
  function contactsImg() {
    ...
    let containerWidth = $(this).width() - ($(this).find(".points__contact-img").css("column-gap").replace(/\D+/g,"") * 2)
    let counter = 0;
    let itemWidthFor = 0;
...
, {
			...
			for (var i = 0; i < markers.length; i++) {
                ...
                '<div class="ballon">'+
						...
						'<div class="ballon__head-text">Suyum</div>'+
                        '</div>'+
                        '<div href="#" class="baloon__body">'+
                        ...
                        //   '<div class="baloon__text-title">Преображенская Старообрядческая Московская Община</div>'+
                          '<div class="baloon__props">'+
                            '<div class="baloon__line">'+
                              '<svg class="icon icon-phone"><use xlink:href="#icon-map-phone"></use></svg>'+
                              '<a href="tel:+79161712767">+7 (916) 171-27-67</a></br>'+
                              '<a href="tel:+79037397246">+7 (903) 739-72-46</a></br>'+
                              '<a href="tel:+79687212251">+7 (968) 721-22-51</a></br>'+
                              '<a href="tel:+79175284157">+7 (917) 528-41-57</a>'+
                            '</div>'+
                            '<div class="baloon__line">'+
                              '<svg class="icon icon-ico_mail2"><use xlink:href="#icon-map-mail"></use></svg>'+
                              '<a href="mailto:frolova.antonina.voda@yandex.ru">frolova.antonina.voda@yandex.ru</a>'+
                            '</div>'+
                            '<div class="baloon__line">'+
                              '<svg class="icon icon-worktime"><use xlink:href="#icon-map-time"></use></svg>'+
                              '<span>Ежедневно с 9:00 до 18:00 </span>'+
                            '</div>'+
                            '<div class="baloon__line">'+
                              '<svg class="icon icon-address"><use xlink:href="#icon-map-address"></use></svg>'+
                              '<span>г. Подольск, ул. Народная, д. 26 б, пав. №2</span>'+
                            '</div>'+
                          '</div>'+
                        '</div>'+
                      '</div>';
